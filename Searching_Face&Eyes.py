...
while success:
	...
	noseRects = noseClassifier.detectMultiScale(image,1.2,2,cv2.CASCADE_SCALE_IMAGE,minEyeSize,maxEyeSize)
	if len(noseRects) > 0:
		for noseRect in noseRects:
			nx,ny,nw,nh=noseRect
			cv2.circle(frame,(nx + nw/2,ny + nh/2),min(nw/2,nh/2),(0,255,0))
	#else: 
